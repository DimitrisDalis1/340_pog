...
| BREAK SEMICOLON
{
	if (check_for_valid_loop_stop(loopcounter) == 0)
		printf("Break not inside of loop in line %d\n", yylineno);
...
{
	if (check_for_valid_loop_stop(loopcounter) == 0)
		printf("Continue not inside of loop in line %d\n", yylineno);
	$1 = malloc(sizeof(stmt_t));
	make_stmt($1);
...
expr
{
	...
	emit(if_eq, NULL, $1, $4, 0, currQuad);
	emit(jump, NULL, NULL, NULL, 0, currQuad);
}
| expr NOT_EQUAL {}
expr
{
	...
	emit(if_noteq, NULL, $1, $4, 0, currQuad);
	emit(jump, NULL, NULL, NULL, 0, currQuad);
};
...
| PLUS2 lvalue
{
	...
	{
		$$ = emit_iftableitem($2);
		emit(add, $$, $$, newexpr_constint(1), -1, currQuad);
...
funcblockend
{
	...
	emit(funcend, $$, NULL, NULL, -1, currQuad);
	$$ = $1;
...
| idlist COMMA ID
{
	...
	else
	{
		...
		$$ = $1;
		SymbolTableEntry *temp;
		temp = SymTable_insert(hash, (const char *)$3, yylineno, NULL, current_scope, FORMAL); 
		temp->offset = formalArgOffset++;
		temp->space = formalarg;
